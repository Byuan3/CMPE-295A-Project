        ...
        else
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    var numBytes = serverSocket.Receive(bytes);
        ...
        if (isConnected)
        {
            try
            {
                ...
                if (fileBytes != null)
                {
                    ...
                    serverSocket.Send(Encoding.ASCII.GetBytes(fileLength));
    ...
    async public static void ReadScreen()
    {
        if (isConnected)
        {
            try
            {
                ...
                serverSocket.Send(Encoding.ASCII.GetBytes(strLength));
    ...
    public static void sendFileBytes(int imageBytesLength, byte[] imageBytes)
    {
        if (isConnected)
        {
            try
            {
                ...
                {
                    serverSocket.Send(imageBytes, i, 1024, SocketFlags.None);
                ...
                if (last != 0)
                {
                    serverSocket.Send(imageBytes, imageBytesLength - last, last, SocketFlags.None);
